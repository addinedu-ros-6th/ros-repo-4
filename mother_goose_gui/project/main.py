...
def robot_monitor_process(robot_name, domain_id, status_queue):
    # 도메인 ID 설정
    ...
    class RobotConnectionNode(Node):
        def __init__(self, robot_name, status_queue):
    ...
    # 노드 생성 및 스피닝
    node = RobotConnectionNode(robot_name, status_queue)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
    ...
    def __init__(self, parent=None):
        ...
        for robot_name, info in self.robot_info.items():
            p = multiprocessing.Process(
                ...
                args=(robot_name, info['domain_id'], self.status_queue),
                daemon=True)
            p.start()
