...
class MainWindow(QMainWindow):
    ...
    def check_position_queue(self):
        while not self.position_queue.empty():
            ...
            print(f"[Main Process] Received position update from {robot_name}: x={x}, y={y}")
        # 위치 업데이트 후 맵 다시 그리기
        self.update_display()
    ...
    def draw_robot(self):
        ...
        painter = QtGui.QPainter(updated_map)

        for robot_name, (x, y) in self.robot_positions.items():
            # 로봇 색상 설정
            if robot_name == 'robot1':
                color = QtGui.QColor(255, 0, 0)  # 빨간색
            elif robot_name == 'robot2':
                color = QtGui.QColor(0, 255, 0)  # 초록색
            else:
                color = QtGui.QColor(0, 0, 255)  # 파란색
