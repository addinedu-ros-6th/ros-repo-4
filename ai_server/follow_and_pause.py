    ...
    def run(self, image):
        ...
        if self.sub_mode < 4:
            ...
            if self.body_size > INITIAL_BODY_SIZE:
                if self.body_center[0] - (center_x - INITIAL_BIAS_LIMIT) < 0:
                    self.sub_mode = 2
                    self.img = cv2.putText(image, "Please move to left side.", (16, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
                elif self.body_center[0] - (center_x + INITIAL_BIAS_LIMIT) > 0:
                    self.sub_mode = 3
                    self.img = cv2.putText(image, "Please move to right side.", (16, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
                else:
                    self.img = self.sub_mode = 4
                    cv2.putText(image, "I recognized you! Let's go!", (16, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
            # 화면 내 상체 크기가 일정 이하인 경우
            else:
                self.sub_mode = 1
                self.img = cv2.putText(image, "Please come a little closer.", (16, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
        ...
        # ---- 주행 단계 (인식 후 following or guiding) ----
            ...
            if self.failed_counter == 0:
                ...
                # pause 손동작이 검출된 경우, self.pause 를 True 로 변환
                hand_check_img = self.hand_detector(image)

                if self.pause:
                    image = cv2.putText(hand_check_img, "Pause", (16, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
                else:
                    image = cv2.putText(hand_check_img, "Following", (16, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
            ...
            # 이전에 저장된 프레임을 사용하여 추적하고 있는 상태
                ...
                self.img = cv2.putText(self.img, "Following(prev_frame)", (16, 16), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1, cv2.LINE_AA)
            else:                    
                self.body_detector_reset(image)
        # ----------------------------------------------
        ...
        send_img = cv2.putText(image, text, (16, 32), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)

        return send_img, self.sub_mode, diff_x, diff_y, self.body_size, self.pause
    ...
    def hand_detector(self, image):
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                gesture_name = self.le.inverse_transform([gesture])[0]

                if gesture_name == HAND_LABELS[0]:
                    self.pause = True

                # 예측 결과를 이미지에 표시
                cv2.putText(image, f'Order: {gesture_name}', (16, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 1, cv2.LINE_AA)

        return image

        
    def body_detector_reset(self, image):
        self.sub_mode = 0  
        self.failed_counter = 0
...
def main():
    ...
    while cap.isOpened():
        ...
        img, sub_mode, diff_x, diff_y, body_size, pause = follower.run(image)
        # img = follower.hand_detector(send_img)
