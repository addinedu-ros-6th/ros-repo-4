...
class PlanPathClient:
    ...
    def get_result_callback(self, future):
        ...
        # 최종 목적지 도착 시 초기화
        if self.node.destination_reached:
...
class NavigateWaypointsTree(Node):
    def __init__(self, yaml_path):
        ...
        self.destination_reached = False  # 최종 목적지 도착 여부
    ...
    def filter_waypoints_near_path(self):
        ...
        buffer_distance = 0.5  # 경로에서 경유지를 포함할 거리 설정
        ...
        # 기존의 트리에 추가하거나 교체
        if len(self.behaviour_tree.root.children) > 1:
            self.behaviour_tree.root.children[1] = waypoints_sequence
        else:
    ...
    def visualize_filtered_waypoints(self):
        ...
        for i, waypoint in enumerate(self.filtered_waypoints):
            ...
            marker.pose.position.x = waypoint['point']['x']
            marker.pose.position.y = waypoint['point']['y']
            marker.pose.position.z = 0.2  # 지면에서 약간 띄우기
            marker.scale.x = 0.2
            marker.scale.y = 0.2
            marker.scale.z = 0.2
            marker.color.a = 1.0
            ...
            if i == self.current_waypoint_index:
                marker.color.r = 1.0  # 빨간색: 현재 경유지
            elif i in self.completed_indices:
                marker.color.g = 1.0  # 초록색: 완료된 경유지
            else:
                marker.color.b = 1.0  # 파란색: 이동 예정 경유지
            ...
            text_marker.pose.position.z = 0.4  # 구 마커 위에 표시
            text_marker.scale.z = 0.2  # 텍스트 크기
            text_marker.color.a = 1.0
            text_marker.color.r = 1.0
            text_marker.color.g = 1.0
            text_marker.color.b = 1.0
            text_marker.id = i + 3000  # 마커 ID가 겹치지 않도록 조정
            text_marker.text = str(i + 1)  # 경유지 번호 표시
            marker_array.markers.append(text_marker)

        # 최종 목적지 표시
        destination_marker = Marker()
        destination_marker.header.frame_id = 'map'
        destination_marker.id = 4000
        destination_marker.type = Marker.CUBE
        destination_marker.pose.position.x = self.destination['x']
        destination_marker.pose.position.y = self.destination['y']
        destination_marker.pose.position.z = 0.2
        ...
        destination_marker.color.b = 0.0  # 노란색
        marker_array.markers.append(destination_marker)

        # 목적지 번호 표시
        dest_text_marker = Marker()
        dest_text_marker.header.frame_id = 'map'
        dest_text_marker.type = Marker.TEXT_VIEW_FACING
        dest_text_marker.action = Marker.ADD
        dest_text_marker.pose.position.x = self.destination['x']
        dest_text_marker.pose.position.y = self.destination['y']
        dest_text_marker.pose.position.z = 0.5
        dest_text_marker.scale.z = 0.3
        dest_text_marker.color.a = 1.0
        dest_text_marker.color.r = dest_text_marker.color.g = dest_text_marker.color.b = 1.0
        dest_text_marker.id = 4001
        dest_text_marker.text = "Goal"
        marker_array.markers.append(dest_text_marker)

        self.marker_publisher.publish(marker_array)
        self.get_logger().info("경유지 상태에 따른 마커가 업데이트되었습니다.")
    ...
    def reset_navigation(self):
        self.get_logger().info("최종 목적지에 도착했습니다. 내비게이션을 초기화합니다.")
        self.goal_received = False
        self.plan_received = False
        self.destination = None
        self.path = None
        ...
        # 마커 초기화
        marker_array = MarkerArray()
        # 기존에 생성된 마커들을 삭제하기 위해 마커들의 action을 DELETE로 설정
        for marker_id in range(1000, 5000):  # 생성된 모든 마커 ID 범위
            marker = Marker()
            marker.header.frame_id = "map"
            marker.id = marker_id
            ...
            marker_array.markers.append(marker)
        self.marker_publisher.publish(marker_array)
        self.get_logger().info("모든 마커가 삭제되었습니다.")
    ...
    def set_destination(self):
        ...
        self.planner_client.send_goal(self.start_pose, self.destination_pose)
        self.get_logger().info(f"경로 계획 요청 전송: {self.destination}")
    ...
    def publish_filtered_waypoints_markers(self):
        ...
        for i, waypoint in enumerate(self.filtered_waypoints):
            ...
            if i == self.current_waypoint_index:
                marker.color.r = 1.0  # 현재 경유지
            elif i in self.completed_indices:
                marker.color.g = 1.0  # 완료된 경유지
            else:
                marker.color.b = 1.0  # 나머지 경유지
        ...
        self.marker_pub.publish(marker_array)
        self.get_logger().info("필터링된 경유지가 시각화되었습니다.")
