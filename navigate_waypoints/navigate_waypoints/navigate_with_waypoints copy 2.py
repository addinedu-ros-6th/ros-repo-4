...
from tf_transformations import quaternion_from_euler  # 쿼터니언 변환 함수
...
# from py_trees_ros.visitors import SnapshotVisitor
...
class NavigateAndVisualizeWaypoints(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        if self.current_index < len(self.node.filtered_waypoints):
            ...
            # === 원하는 마커 ID를 /desired_marker_id 토픽에 발행 ===
            if self.node.current_goal_id is not None:
                desired_marker_id = self.node.current_goal_id  # goal_id 사용
                msg = Int32()
                msg.data = desired_marker_id
                self.node.desired_marker_pub.publish(msg)
                self.node.get_logger().info(f"Published desired_marker_id: {desired_marker_id}")
            else:
                self.node.get_logger().warn("current_goal_id가 설정되지 않았습니다.")
            # === 발행 완료 ===
    ...
    def send_goal(self, waypoint, index, is_final_goal):
        ...
        try:
            ...
            # 방향 벡터의 각도 계산
            dx = waypoint['point']['x'] - robot_x
            dy = waypoint['point']['y'] - robot_y
            yaw = math.atan2(dy, dx)  # 각도 계산
            ...
            self.node.get_logger().error(f"현재 위치를 가져오는 중 오류 발생: {e}")
...
class NavigateWaypointsTree(Node):
    def __init__(self, yaml_path):
        ...
        self.desired_marker_pub = self.create_publisher(Int32, '/desired_marker_id', 10)
        self.current_goal_id = None  # 수신한 goal_id를 저장할 변수

        # Behavior Tree 생성 및 Viewer 서버 실행
        self.behaviour_tree = BehaviourTree(self.root)
        self.behaviour_tree.setup(timeout=15)  # Tree 실행 준비
        self.get_logger().info("Behavior Tree Viewer가 설정되었습니다.")

        # Behavior Tree 시각화 파일 저장 (실행 파일과 동일한 경로)
        output_name = "navigate_waypoints_tree"
        render_dot_tree(self.root, name=output_name)  # target_directory 생략
        self.get_logger().info(f"Behavior Tree 구조를 {output_name}.png로 저장했습니다.")

        # DebugVisitor 추가
        debug_visitor = DebugVisitor()
        self.behaviour_tree.visitors.append(debug_visitor)  # 트리에 DebugVisitor 추가
        self.get_logger().info("DebugVisitor가 추가되었습니다. Tree Viewer에서 트리 상태를 확인할 수 있습니다.")
    ...
    def request_callback(self, msg):
        # "/Request"에서 데이터 읽기
        request_data = msg.data  # 예: "auto_delivery 8"
        try:
            # 숫자만 추출
            goal_id = int(request_data.split()[-1])  # 마지막 숫자 추출
            self.get_logger().info(f"수신된 목표 ID {goal_id}")

            # 기존 goal_callback의 로직 재사용
            self.reset_navigation()
            self.current_goal_id = goal_id
            self.destination = next((goal['point'] for goal in self.goal_points if goal['id'] == goal_id), None)

            if self.destination:
                self.get_logger().info(f"새로운 목적지 설정: {self.destination}")
                self.goal_received = True
            else:
                self.get_logger().error(f"YAML에서 목표 ID {goal_id}를 찾을 수 없습니다.")
        except ValueError as e:
            self.get_logger().error(f"목표 ID 추출 중 오류 발생: {e}")
    ...
    def filter_waypoints_near_path(self):
        ...
        try:
            ...
            robot_position = {'x': 0.0, 'y': 0.0}  # 기본값 설정

        # 경유지를 현재 위치에서 가까운 순서로 정렬
        self.filtered_waypoints.sort(
            key=lambda waypoint: self.calculate_distance(robot_position, waypoint['point'])
        )
    ...
    def reset_navigation(self):
        ...
        self.start_pose = None
        self.destination_pose = None
        self.filtered_waypoints = []  # 필터링된 경유지 초기화
        ...
        self.get_logger().info("모든 마커를 DELETEALL을 사용하여 삭제했습니다.")
    ...
    def setup_behavior_tree(self):
        ...
        self.behaviour_tree = py_trees.trees.BehaviourTree(
            root=self.root
            # unicode_tree_debug=True  # 유니코드 디버그 활성화
        )

        # Snapshot Visitor 관련 코드 삭제
        # snapshot_visitor = SnapshotVisitor()
        # self.behaviour_tree.visitors.append(snapshot_visitor)
        # self.get_logger().info("Snapshot Visitor가 추가되었습니다. 트리 상태가 스트리밍됩니다.")
    ...
    def tick_tree(self):
        if self.bt_setup and self.behaviour_tree is not None:
            ...
            self.get_logger().debug("트리가 한 번 Tick되었습니다.")  # Tick 디버깅 메시지 추가


    def main_loop(self):
        while rclpy.ok():
            self.tick_tree()  # 트리 상태 갱신
...
def main(args=None):
    ...
    package_share = get_package_share_directory('navigate_waypoints')  # 패키지 이름 확인
    yaml_path = os.path.join(package_share, 'navigate_waypoints', 'points.yaml')  # 파일의 상대 경로 설정
