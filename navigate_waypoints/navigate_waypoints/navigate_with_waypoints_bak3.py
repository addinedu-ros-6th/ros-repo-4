...
class NavigateAndVisualizeWaypoints(py_trees.behaviour.Behaviour):
    def __init__(self, name, node):
        ...
        self.action_client = ActionClient(node, NavigateToPose, 'navigate_to_pose')  # NavigateToPose를 위한 액션 클라이언트 생성
        self.goal_sent = False
    ...
    def update(self):
        if self.current_index >= len(self.node.filtered_waypoints):
            self.node.get_logger().info("모든 경유지에 도달했습니다.")
            return py_trees.common.Status.SUCCESS

        if not self.goal_sent:
            waypoint = self.node.filtered_waypoints[self.current_index]
            self.send_goal(waypoint, self.current_index)

        return self.status

    def send_goal(self, waypoint, index):
        goal_pose = PoseStamped()
        goal_pose.header.frame_id = 'map'
        goal_pose.header.stamp = self.node.get_clock().now().to_msg()  # 타임스탬프 설정
        ...
        self.action_client.wait_for_server()  # 서버 대기
        self._send_goal_future = self.action_client.send_goal_async(
            goal_msg, feedback_callback=self.feedback_callback)
        self._send_goal_future.add_done_callback(self.goal_response_callback)
        self.goal_sent = True
