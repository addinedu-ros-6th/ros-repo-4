...
### 2.4 Operating Scenario
![scenario1](https://github.com/user-attachments/assets/2a2071af-91df-4fb5-b0c6-d1760eb2168e)

## 3. 프로젝트 설계
### 3.1 System Architecture
![system_architecture](https://github.com/user-attachments/assets/b104646a-605a-455a-9e72-550979f78594)
...
* 사용자 얼굴 재확인
    - 도착지에서 최초 등록 사용자 얼굴과 일치하는지 확인한다. 
...
* 짐 꺼내기
    ...
    - 내부 프로세스는 앞에서 설명한 `짐싣기` 와 동일하게 동작
* 사용 종료
...
* (기술 조사) Tracker 모델 사용 
  - YOLO Tracker, Deepsort 등의 tracking 딥러닝 모델의 경우, Frame 내 모든 인물에 대한 ID 가 부여됨. 
  - 특정 ID 객체만 추적하기 위해서는 해당 객체의 특징(ex. 얼굴, 의상 등)을 다시 검출, 일치하는 객체를 선별해야 하므로, 얼굴 인식, 의상 검출 등의 추가 모델을 함께 동작해야 함.
  - 정보에 일치하는 인물 추적 구현이 가능해지나, 현재 H/W 성능상 이렇게 복합적으로 구현했을 때, 처리시간이 100ms 이상으로 매 loop 당 로봇이 추적해야 하는 변위가 커지며, Deepsort 모델 내 칼만 필터에 의해 예측된 다음 위치 값에 대한 정확도가 매우 떨어져 추적이 어려워짐을 확인.        
  - 아울러 해당 모델들의 경우, 모든 신체을 포함한 bounding box 를 검출하는데 팔을 벌리거나, 신체를 숙이는 등의 동작에서 bounding box 의 중심좌표가 실제 신체의 무게중심에서 크게 벗어나는 현상이 발생하여, 이동 중심 좌표 검출을 위한 추가 모델 검토가 필요할 수 있음을 확인함.

* (실제 적용) 신체 탐지 및 추적 data 확보를 위한 단일 & 경량화 모델(Mediapipe) 사용 
